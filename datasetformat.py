# change .json file into .arrow file, also optionally merge the data with another .arrow file
import json
import pyarrow as pa
import random

merge_with_full_dataset = False

f=open("sample1k-HCVerifyAll.json", "r")
#f=open("sample1k-HCVerifyModAll.json", "r")
s=json.load(f)
f.close()
...
if merge_with_full_dataset:
    ...
    input_file = './data/validation/data-00000-of-00001.arrow'
    with pa.memory_map(input_file, 'r') as source:
        reader = pa.RecordBatchStreamReader(source)
