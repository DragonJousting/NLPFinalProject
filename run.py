...
from helpers import prepare_dataset_nli, prepare_train_dataset_qa, \
    prepare_validation_dataset_qa, QuestionAnsweringTrainer, compute_accuracy, postprocess_qa_predictions2
import os
import json
...
def main():
    ...
    if args.dataset.endswith('.json') or args.dataset.endswith('.jsonl'):
        ...
        dataset = datasets.load_dataset('json', data_files=args.dataset, trust_remote_code=True)
        ...
        dataset = datasets.load_from_disk(*dataset_id)
    ...
    # attempt to get information out during training to do dataset cartography
    # but I think you would have to subclass the model to be able to get the confidence (perplexity)
    class MyTrainer(QuestionAnsweringTrainer):

        def prediction_step(self, model: nn.Module, inputs: dict, prediction_loss_only: bool, ignore_keys = None ):

            out = super().prediction_step(model, inputs, False, ignore_keys=ignore_keys)
            eval_preds = postprocess_qa_predictions2(eval_dataset,
                                                    self.eval_dataset,
                                                    out[1])
